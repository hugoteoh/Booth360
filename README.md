...
1. 手机 App：设置 → 上传 选「腾讯云 COS」并填好配置；设置 → 打开「局域网控制」
2. 大屏电脑与手机连**同一 Wi-Fi**（或电脑直连手机的个人热点），浏览器打开设置页显示的「大屏展示页」地址（`http://<手机IP>:8360/wall`），按 **F11** 全屏投到大电视
3. 嘉宾拍完 → 视频**几秒内**自动出现在大屏循环播放（走局域网不等云端）→ COS 上传完成后旁边**自动浮出二维码** → 嘉宾扫码下载
4. 右侧是历史列表，点任意一条可手动回放展示；45 秒无操作自动回到最新
5. `http://<手机IP>:8360/`（不带 /wall）是操作员控制台：看状态、切活动、远程开拍
