...
**共同准备**：手机 App 设置 → 上传选「腾讯云 COS」并填好配置。

### 模式一：局域网大屏（同一 Wi-Fi / 手机热点，上屏最快）
1. 设置 → 打开「局域网控制」
2. 大屏电脑浏览器打开「大屏展示页」地址（`http://<手机IP>:8360/wall`）→ **F11** 全屏
3. 嘉宾拍完 → 视频**几秒内**自动上屏循环播放（走局域网不等云端）→ 上传完成后旁边**自动浮出二维码**
4. `http://<手机IP>:8360/`（不带 /wall）是操作员控制台：看状态、切活动、远程开拍

### 模式二：云端大屏（大屏和手机**不用同一网络**，异地也行）
1. 设置 → 上传 里打开「云端大屏」开关（默认开）
2. 任何有网的电脑打开固定地址 `https://<你的桶>.cos.<区域>.myqcloud.com/booth360/wall/index.html` → F11 全屏
3. 每段视频**上传完成后**自动出现在该页（约拍完后 10–60 秒，视网速）；二维码同屏
4. 页面公开可读（知道链接即可看）；活动结束在 COS 控制台删掉 `booth360/wall/` 目录即可下架

两种模式界面一致：左侧最新视频大屏循环，右侧大二维码 + 历史列表（可点选回放，45 秒后自动回最新）。
